...
# FORK: repo identity comes from one place (see hermes_fork.py); it imports only
# __future__, so it cannot cycle.
from hermes_fork import FORK_PUBLISHES_SKILLS_INDEX as _FORK_PUBLISHES_SKILLS_INDEX
from hermes_fork import FORK_SLUG as _FORK_SLUG
...
class OptionalSkillSource(SkillSource):
    ...
    # FORK: this labels where the optional-skills/ tree that ships in THIS repo
    # came from, and the centralized index consumes it to build source URLs.
    # It has to name the fork, or "install this official skill" resolves against
    # a repo whose optional-skills/ may not match what is on disk here.
    OFFICIAL_REPO = _FORK_SLUG
...
# The index is generated by scripts/build_skills_index.py and committed to
# website/static/api/skills-index.json, so this URL resolves without needing a
# hosted website. Regenerate it when optional-skills/ changes.
HERMES_INDEX_URL = _fork_raw_url("website/static/api/skills-index.json")
...
def _load_hermes_index() -> Optional[dict]:
    ...
    FORK: returns None unless this fork publishes the index. Upstream serves it
    from its own docs site; fetching from there is exactly the upstream coupling
    this fork removed, and pointing at the fork's raw URL while the file is not
    committed would only mean a guaranteed 404 every TTL. Skipping the request is
    the honest behaviour -- see FORK_PUBLISHES_SKILLS_INDEX in hermes_fork.py for
    how to turn it on, and note the other ten skill sources are unaffected.
    """
    if not _FORK_PUBLISHES_SKILLS_INDEX:
        return None
